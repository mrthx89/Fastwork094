...
{
    public partial class frmEntriStokMasterData : DevExpress.XtraEditors.XtraForm
    {
        public Model.ViewModel.StokMasterData data = null;
        public frmEntriStokMasterData(Model.ViewModel.StokMasterData data)
        {
            InitializeComponent();
        ...
        private void frmEntriStokMasterData_Load(object sender, EventArgs e)
        {
            ...
            if (data == null)
            {
                data = new Model.ViewModel.StokMasterData()
                {
                    ID = Guid.NewGuid(),
            ...
            {
                data.Saldo = Repository.StokKeluar.getSaldoStok(data.IDInventor, data.Tanggal, data.ID, Constant.TypeTransaction.stokMasterData).Item2;
            }
            refreshLookUp();
            StokMasterDataBindingSource.DataSource = data;
        ...
        private void mnSimpan_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            using (WaitDialogForm dlg = new WaitDialogForm("Sedang menyimpan data"))
            {
                try
                {
                    ...
                    {
                        var save = Repository.StokMasterData.saveStokMasterData(data);
                        if (save.Item1)
                        {
        ...
        private void refreshLookUp()
        {
            using (WaitDialogForm dlg = new WaitDialogForm("Sedang mengambil data"))
            {
                try
                {
                    ...
                    var callItems = Repository.Item.getLookUpInventors(data.Tanggal, data.IDWarehouse, (data != null ? data.ID : Guid.Empty), null);
                    if (callItems.Item1)
                    {
        ...
        private void frmEntriStokMasterData_FOrmClosing(object sender, FormClosingEventArgs e)
        {
        ...
        private void TanggalDateEdit_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                data.Saldo = Repository.StokKeluar.getSaldoStok(data.IDInventor, TanggalDateEdit.DateTime, data.ID, Constant.TypeTransaction.stokMasterData).Item2;
