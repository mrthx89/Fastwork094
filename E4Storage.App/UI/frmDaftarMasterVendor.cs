...
{
    public partial class frmDaftarMasterVendor : DevExpress.XtraEditors.XtraForm
    {
        public frmDaftarMasterVendor()
        {
            InitializeComponent();
        }
        ...
        private void frmDaftarMasterVendor_Load(object sender, EventArgs e)
        {
            mnRefresh.PerformClick();
        }

        private void frmDaftarMasterVendor_FormCLossing(object sender, FormClosingEventArgs e)
        {
        ...
        private void mnEdit_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                var item = (VendorMaster)tVendorBindingSource.Current;
                if (item != null)
                {
                    addOrEdit(item);
                }
        ...
        private void addOrEdit(VendorMaster data)
        {
            using (frmEntriVendor frm = new frmEntriVendor(data))
            {
                try
                {
        ...
        private void mnHapus_ItemClick(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            try
            {
                ...
                if (item != null)
                {
                    if (MsgBoxHelper.MsgQuestionYesNo($"{this.Name}.mnHapus_ItemClick", $"Yakin ingin menghapus supplier {item.Name} ini?") == DialogResult.Yes)
                    {
                        var delete = Repository.Vendor.deleteVendor(item.ID);
                        if (delete.Item1)
                        {
                            mnRefresh.PerformClick();
                        }
        ...
        private List<VendorMaster> data = new List<VendorMaster>();
        private void refreshData()
        {
            using (WaitDialogForm dlg = new WaitDialogForm("Sedang merefresh data"))
            {
                try
                {
                    ...
                    var call = Repository.Vendor.getVendor(null);
                    if (call.Item1)
                    {
                        data = call.Item2;
                    }
                    tVendorBindingSource.DataSource = data;
