...
{
    public partial class dlgSummaryItemMasuk : DevExpress.XtraEditors.XtraUserControl
    {
        private DateTime TglDari, TglSampai;
        public dlgSummaryItemMasuk(DateTime TglDari, DateTime TglSampai)
        {
            InitializeComponent();
        ...
        private void simpleButton1_Click(object sender, EventArgs e)
        {
            using (WaitDialogForm dlg = new WaitDialogForm("Sedang merefresh data"))
            {
                try
                {
                    var data = Repository.StokMasuk.getStokMasuks(dateEdit1.DateTime, dateEdit2.DateTime);
                    if (data.Item1)
                    {
